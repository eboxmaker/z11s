...
static void onUI_show() {
	...
	mTextPersonNamePtr->setText("");
	mTextPersonIDPtr->setText("");
...
static void onProtocolNetDataUpdate(const NetProtocolData &data)
{
	...
	if(data.cmd == Cmd::PersonGet)
	{
		parsePersonGet(data, person);
		mTextPersonNamePtr->setText(person.name);
		mTextPersonIDPtr->setText(person.id);
		mTextPersonLevelPtr->setText(person.level);
		mTextLockRightPtr->setText(person.lockCtr);
		mTextFingerNumPtr->setText((int)person.fingers.size());
	}
}
/**
 * 定时器触发函数
...
static bool onButtonClick_BtnQuaryPerson(ZKButton *pButton) {
    NetProtocolData msg;
    msg = makePersonGet(Status::Get, mEditPersonIDPtr->getText());
    netUser.write(msg);
...
static bool onButtonClick_BtnRollStep(ZKButton *pButton) {
	...
	person.fingers.push_back("123123wfsdvfasdf");
    return false;
}
...
static bool onButtonClick_BtnUpdateServer(ZKButton *pButton) {
    ...
    NetProtocolData msg;
    msg = makePersonUpdate(Status::Set, person);
    netUser.write(msg);
